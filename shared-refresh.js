...
(() => {
  if (location.pathname.startsWith('/admin')) return;
  if (document.getElementById('lang-toggle-btn') || document.getElementById('mast-stat')) return;

  const STAT_URL = './site-stats.json';
  const REFRESH_MS = 60_000;
  const INITIAL_FETCH_DELAY_MS = 1_500;
  const mobileQuery = window.matchMedia('(max-width: 640px)');
  const fetchJson = window.leviathanData?.fetchJson || ((url) => fetch(url, {
    cache: 'default',
